...
function renderReachabilityTree(interpretedTree) {
    ...
    function renderTree(root, index) {
        ...
        linkGroups
            ...
            .style("font-size", "25px")
            .style("fill", "#000")
            .text((d) =>d.target.data.transition ? d.target.data.transition : "");
...
//     }
    ...
    //console.log(`O ID do node relacionado é: ${nodeIds.join(', ')}`);
...
//         // Variável para verificar se há nó InfinityPseudoNode
//         let hasInfinityPseudoNode = false;
        
//         hasInfinityPseudoNode = infitytree;
        
//         console.log(hasInfinityPseudoNode);
//         // // Verifica se há nó InfinityPseudoNode na árvore
//         // treeData.descendants().forEach((d) => {
//         //     //console.log("Node state:", d.data.transition); // Adicionando log para cada nó
//         //     if (d.data.data === "InfinityPseudoNode") {
//         //         hasInfinityPseudoNode = true;
//         //     }
//         // });
    
//         //console.log("Has InfinityPseudoNode:", hasInfinityPseudoNode);
    
//         // Se houver InfinityPseudoNode
//         if (hasInfinityPseudoNode) {
//             console.log("Entering InfinityPseudoNode handling");
    
//             // Mapeamento de transições repetidas
//             const transitionMap = new Map();
    
//             // Filtra transições repetidas
//             treeData.descendants().forEach((d) => {
//                 const transition = d.data.transition;
//                 if (transition) {
//                     if (transitionMap.has(transition)) {
//                         transitionMap.set(transition, transitionMap.get(transition) + 1);
//                     } else {
//                         transitionMap.set(transition, 1);
//                     }
//                 }
//             });
    
//             // Define um conjunto de transições únicas
//             const uniqueTransitions = new Set();
    
//             // Adiciona transições únicas ao conjunto
...
//             // Adiciona um nó InfinityPseudoNode
//             const pseudoNode = {
//                 state: "InfinityPseudoNode",
//                 id: "InfinityPseudoNode",
//                 transition: "All Transitions"
//             };
    
//             // Adiciona o nó InfinityPseudoNode ao início dos nós
//             treeData.descendants().unshift({data: pseudoNode, depth: 0, height: 0, parent: null, x: 0, y: 0});
    
//             // Adiciona uma única transição repetida apontando para InfinityPseudoNode
//             treeData.descendants().forEach((d) => {
//                 const transition = d.data.transition;
//                 if (transition && !uniqueTransitions.has(transition)) {
//                     d.data.transition = null; // Limpa a transição
//                     d.parent = treeData.descendants()[0]; // Aponta para InfinityPseudoNode
//                 }
//             });
//         }
...
//             .enter()
//             .filter((d) => d.data.state !== "Deadlock")
//             .filter((d) => d.data.state !== "InfinityPseudoNode")
//             .append("g")
//             .attr("class", `node node-${index}`)
//             .attr("transform", (d) => {
//                 //Define uma chave para o node para facilitar a busca na arvore e verificar a repetição
...
//         //Define o formato do nó no caso é um circulo de cor branca sem bordas
//         nodes
//             .append("circle")
//             .attr("r", 28)
//             .style("stroke", "steelblue")
